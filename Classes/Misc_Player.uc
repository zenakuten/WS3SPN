...
replication
{
    ...
    reliable if(Role < ROLE_Authority)
		...
		ServerLoadSettings, ServerSaveSettings, ServerReportNewNetStats,ServerSetEyeHeightAlgorithm, ServerSetNetUpdateRate;
...
function ServerSetNetUpdateRate(float Rate, int NetSpeed) {
    ...
    MaxRate = RepInfo.MaxNetUpdateRate;
    if (NetSpeed != 0)
        MaxRate = FMin(MaxRate, NetSpeed/100.0);

    MinRate = RepInfo.MinNetUpdateRate;
...
function UTComp_ReplicateMove(
    ...
    if (PendingMove == None) {
        ...
        if(RepInfo != None && class'Misc_Player'.default.bEnableEnhancedNetCode)
        {
